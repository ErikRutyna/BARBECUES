...
from mesh_refinement import reorient_ccw
...
def main():
    ...
    bbox_walls = dm.sqr_bbox_wall_gen(2*L, 2*L, 0.2)

    # Circle
    # interior_walls = circle_bbox_wall_gen(0.25, 15)

    # Flat Plate
    interior_walls = dm.sqr_bbox_wall_gen(2, 0.2, 0.1)
    ...
    sdf = lambda p: dm.ddiff(dm.dpoly(bbox_walls, p),
                             dm.dpoly(interior_walls, p))

    # Generate nodes and cells in the domain via a level-set based algorithm
    V, T = dm.distmesh2d(sdf, 0.15, bbox_out, np.vstack((bbox_walls, interior_walls)), 0.2, 1.2, 6e-1, 1e-3)

    # Writes the mesh out to the "Meshes" directory
    write_mesh(bbox_out, V, T, internal_walls(interior_walls, True), 'demo_plate.gri')
