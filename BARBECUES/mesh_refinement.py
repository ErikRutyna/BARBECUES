...
def find_uniform_splitting(state, mesh, y, f):
    ...
    for i in range(len(mesh['BE'])):
        ...
        if mesh['Bname'][be[3]] == 'Inflow' or mesh['Bname'][be[3]] == 'Outflow':
            ...
            flux_c = flux.stateFluxEuler2D(state[be[2]], y)
    ...
    for i in range(len(mesh['IE'])):
        ...
        flux_l = flux.stateFluxEuler2D(u_l, y)
        ...
        flux_r = flux.stateFluxEuler2D(u_r, y)
...
def find_flagged_edges(state, E, V, IE, BE, f, y):
    ...
    for i in range(BE.shape[0]):
        # Skip over the boundary edges that are supersonic inflows/outflows - don't need to refine them
        if BE[i, 3] == 2 or BE[i, 3] == 3: error[IE.shape[0] + i, :] = np.array((0, BE[i, 2], -1)); continue
        # For all other boundary edges, calculated the error based on the Mach number normal to the local BE
